...
describe('DashboardHero', () => {
	...
	it('renders KPI metrics when stats are available', () => {
		...
		expect(
			screen.getByTestId('dashboard-hero-metric-drafts-awaiting-review'),
		).toHaveTextContent('5')
	...
	it('renders empty state guidance when no documents exist', () => {
		...
		expect(
			screen.getByRole('button', { name: /add first metadata document/i }),
		).toBeInTheDocument()
		expect(
			screen.queryByTestId('dashboard-hero-metric-total-documents'),
